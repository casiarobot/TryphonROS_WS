...
ModbusSend( LtByte aFunction, LtByte *aBuffer, LtByte aLength, LtByte aAddressI2C, LtHandle aHandle, LtByte &aLastFunction)
{
	...
	if ( WriteToSerialPort(aAddressI2C, aHandle, lMessage, aLength ) == aLength )
    {
		aLastFunction = aFunction;
        return LT_SUCCESS;
    }
...
ModbusReceive(LtByte *aBuffer, LtByte aAddressI2C, size_t aLenMessage, LtHandle aHandle, LtByte &aLastFunction)
{
	...
	lRead = ReadFromSerialPort(aAddressI2C, aHandle, lMessage, aLenMessage);
...
ModbusConnect(LtByte aAddressI2C, LtHandle aHandle, LtByte &aLastFunction){
	LtResult lResult = ModbusSend( MODBUS_SERVER_ID, NULL, 0, aAddressI2C, aHandle, aLastFunction);

	if ( lResult == LT_SUCCESS ){
		LtByte lDummy[MODBUS_MAX_PAYLOAD];

		lResult = ModbusReceive(lDummy, aAddressI2C, 56, aHandle, aLastFunction);
...
LtResult
ModbusReadInputRegisters( LtU16 aNo, LtU16 aCount, LtU16 *aValues, LtByte aAddressI2C, LtHandle aHandle, LtByte &aLastFunction)
{
	...
	lResult = ModbusSend( 4, lPayload, 4, aAddressI2C, aHandle, aLastFunction);
    
    if ( lResult == LT_SUCCESS )
    {
        //printf("[Modbus] ModbusReadInputRegisters-send success\n");
		lResult = ModbusReceive( lPayload, aAddressI2C, 255, aHandle, aLastFunction);

        if ( lResult >= 0 )
        {
            LtU16 i=0;

            //printf("[Modbus] ModbusReadInputRegisters-receive success\n");
            for( ; i<aCount; ++i )
            {
...
LtResult
ModbusReadHoldingRegister( LtU16 aNo, LtU16 *aValue, LtByte aAddressI2C, LtHandle aHandle, LtByte &aLastFunction)
{
    LtByte   lPayload[MODBUS_MAX_PAYLOAD];
    LtResult lResult;
	...
	lResult = ModbusSend( 3, lPayload, 4, aAddressI2C, aHandle, aLastFunction);
    
    if ( lResult == LT_SUCCESS )
    {
		lResult = ModbusReceive( lPayload, aAddressI2C, 256, aHandle, aLastFunction);
...
LtResult
ModbusWriteRegister( LtU16 aNo, LtU16 aValue, LtByte aAddressI2C, LtHandle aHandle, LtByte &aLastFunction)
{
    LtByte   lPayload[MODBUS_MAX_PAYLOAD];
    LtResult lResult;
	...
	lResult = ModbusSend( 6, lPayload, 4, aAddressI2C, aHandle, aLastFunction);
