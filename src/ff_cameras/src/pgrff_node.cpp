...
{	
    ...
    ros::NodeHandle n("camera1");
    ros::NodeHandle n2("camera2");
    ...
    camera_info_manager::CameraInfoManager cinfo(n,frame_id);  ///< Helper class to manage the calibration info for the camera
    ros:: Publisher pubinfo = n.advertise<sensor_msgs::CameraInfo>("/camera1/camera_info", 1);
   ...
   std::string url="file:///home/py/ROSTryphon-groundstation/src/ff_cameras/launch/calibration/camera1.yaml";
    //n.getParam("camera_info_url", url);
    cinfo.loadCameraInfo(url);
    std::string url2="file:///home/py/ROSTryphon-groundstation/src/ff_cameras/launch/calibration/camera2.yaml";
...
while(ros::ok())
{
        ...
        retVal2 = PollForTriggerReady( &cam2 );
        ...
        if (grabImage(image,&cam2,frame_id2))
        {
            printf( "Image grabbed \n" );
            ci=cinfo2.getCameraInfo();
            ci.header.stamp = now;
            ci.header.frame_id = frame_id2;
            it_pub2.publish (image);          //publish the image
