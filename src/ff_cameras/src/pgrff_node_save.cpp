...
{	
	...
	camera_info_manager::CameraInfoManager cinfo(n);  ///< Helper class to manage the calibration info for the camera
    ...
    ros::Publisher pubinfo2 = n.advertise<sensor_msgs::CameraInfo>("/camera2/camera_info", 1);
...
while(ros::ok())
{
        ...
        if (grabImage(image,&cam,frame_id))
        {
            ...
            //sensor_msgs::ImagePtr send(new sensor_msgs::Image(image));
        ...
        {
            printf( "Image grabbed \n" );
            ci.reset(new sensor_msgs::CameraInfo(cinfo2.getCameraInfo()));
            ci->header.stamp = image.header.stamp;
            ci->header.frame_id = image.header.frame_id;
            //sensor_msgs::ImagePtr send(new sensor_msgs::Image(image));
