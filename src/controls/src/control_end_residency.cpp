...
void updateVels( double (&v)[6][avgNb])
{
  for(int i=0;i<6;i++)
  {
    for(int k=0;k<avgNb-1;k++)
    {
      v[i][k]=v[i][k+1];
    }
  }
...
Eigen::Vector3d getLinearVel(double v[6][avgNb])
{
  Eigen::Vector3d lv(0,0,0);
  for(int k=0;k<avgNb;k++)
  {
    lv(0)+=(double)v[0][k]/avgNb;
    lv(1)+=(double)v[1][k]/avgNb;
    lv(2)+=(double)v[2][k]/avgNb;
  }
  return lv;
}


Eigen::Vector3d getAngularVel(double v[6][avgNb])
{
  Eigen::Vector3d av(0,0,0);
  for(int k=0;k<avgNb;k++)
  {
    av(0)+=(double)v[3][k]/avgNb;
    av(1)+=(double)v[4][k]/avgNb;
    av(2)+=(double)v[5][k]/avgNb;
  }
  return av;
}

void setLinearVel(double (&v)[6][avgNb], Eigen::Vector3d lv)
{
...
void setAngularVel(double (&v)[6][avgNb], Eigen::Vector3d av)
{
...
void subVel(const geometry_msgs::TwistStamped Velocities)
{
  ...
  if(!start)
  {
    Eigen::Vector3d velTemp, avelTemp;
    velTemp(0)=Vel.linear.x; // defined in global frame
    velTemp(1)=Vel.linear.y;
    velTemp(2)=Vel.linear.z;
    avelTemp(0)=Vel.angular.x; // defined in IMU frame
    avelTemp(1)=Vel.angular.y;
    avelTemp(2)=Vel.angular.z;
    avelTemp=RIMUmatrix*avelTemp;  // defined in body frame

    velTemp=velTemp+Rmatrix*CPCMIMUmatrix*avelTemp; // compute the vel of the center of mass

    setLinearVel(vels,velTemp);
    setAngularVel(vels,avelTemp);

    vel=getLinearVel(vels);
    avel=getAngularVel(vels);
