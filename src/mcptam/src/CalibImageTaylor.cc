...
void CalibImageTaylor::DrawImageGrid()
{
  ...
  for(int i=0; i< (int) mvGridCorners.size(); i++)
  {
    ...
    for(int dirn=0; dirn<4; dirn++)
    {
      if(mvGridCorners[i].maNeighborStates[dirn].val > i)
      {
        ...
        CVD::glVertex(mvGridCorners[mvGridCorners[i].maNeighborStates[dirn].val].mParams.v2Pos + CVD::vec(mirDrawOffset));
      }
    }
  }
  glEnd();
  ...
  for(unsigned int i=0; i<mvGridCorners.size(); i++)
  {
    ...
    mpGLWindow->PrintString(mirDrawOffset + CVD::ImageRef(5,-5) + CVD::ir_rounded(mvGridCorners[i].mParams.v2Pos), ss.str(), 7);
  }
}

// Draw the current grid (as defined by the current found checkerboard corners), projected into a given camera at the current pose of the CalibImageTaylor
...
// Degree is degree of polynomial that will be built into the matrix, nImgNum is the number of this image in the overall sequence of nTotalNum
// Assumes that the current mse3CamFromWorld has been updated, so that the column vectors of the rotation matrix can be extracted as well as
...
std::pair< Matrix<>, Vector<> > CalibImageTaylor::BuildIntrinsicMatrixEntries(Vector<2> v2Center, int nDegree, int nImgNum, int nTotalNum)
{
...
void CalibImageTaylor::GuessInitialPose(TooN::Vector<2> v2Center)
{
  ...
  */
  ...
  // The above else if condition was producing bad results when the origin of the grid was very close to the center of the image (< 0.5 pixels)
  // and the least squares fit solved for the t1 and/or t2 vectors with the wrong sign (but very small magnitude). In this case the assertion
...
// Project the found corners using the given camera model, but return only only the projection errors without any jacobians
...
std::vector<CalibImageTaylor::ErrorAndJacobians> CalibImageTaylor::Project(TaylorCamera &camera)
{
  ...
  for(unsigned int n=0; n<mvGridCorners.size(); n++)
  {
    ...
    {
      // Get the motion of the point in the camera's frame when the pose of the camera changes by one of the degrees of freedom
