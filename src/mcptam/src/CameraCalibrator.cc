  ...
  while(!mbDone)
  {
    ...
    else
    {
      ...
      ost << "(That would be a pixel aspect ratio of " 
          <<  mpCamera->PixelAspectRatio() << ")" << std::endl;
...
bool CameraCalibrator::OptimizeOneStepLM()
{
  ...
  for(int n=0; n<nViews; n++)
  {
    mvpCalibImgs[n]->mse3CamFromWorldNew = SE3<>::exp(vCameraPoseUpdate.slice(n*6, 6)) * mvpCalibImgs[n]->mse3CamFromWorld;
  }
...
void CameraCalibrator::FindBestCenter(Vector<2> v2StartCenter, Vector<2> v2Spread, CVD::ImageRef irNumPoints, Vector<2>& v2BestPos, Vector<4>& v4BestParams, double& dMinError)
{
...
TooN::Vector<4> CameraCalibrator::ComputeParamsUpdatePoses(std::vector<CalibImageTaylor*> vpCalibImgs, TooN::Vector<2> v2Center)
{
  ...
  for(int i=0; i < nViews; ++i)
  {
    ...
    vpCalibImgs[i]->GuessInitialPose(v2Center);  // Update internal pose, make sure this is called before building big matrix!
  }
  ...
  for(int i=0; i < nViews; ++i)
  {
    std::pair< TooN::Matrix<>, TooN::Vector<> > MandB = vpCalibImgs[i]->BuildIntrinsicMatrixEntries(v2Center, 4, i, nViews);
