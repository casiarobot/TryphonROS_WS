...
, mNodeHandlePriv("~")
{
  ...
  for(unsigned i=0; i < vCamGroupStrings.size(); ++i)
  {
    ...
    mvpCamGroups.push_back(new CameraGroupSubscriber(vCamGroupStrings[i], bGetPoseSeparately));  // new subscriber group (with pose subscription)
  ...
  for(unsigned i=0; i < mvpCamGroups.size(); ++i)
  {
    ImagePtrMap mpImages = mvpCamGroups[i]->GetNewImage(&mbWaitActive); // get a set of images so we can extract image size info
...
// 8 - affine transform param e
  ...
  if(!bGetPoseSeparately)  // pose is located in the info message, so extract it
  {
    ...
    if(v3Diff * v3Diff > 1e-4)
    {
      m3Rot = Identity;
      v3Trans = Zeros;
      ROS_WARN_STREAM("VideoSourceMulti: The rotation matrix inside the CameraInfo message of "<<cameraName<<" is invalid!");
...
bool VideoSourceMulti::GetAndFillFrameBWandRGB(std::string camName, ros::WallDuration timeout, CVD::Image<CVD::byte> &imBW, CVD::Image<CVD::Rgb<CVD::byte> > &imRGB)
{
...
// Acquires a new set of images from the camera group whose data arrives first, outputs it in greyscale and color RGB formats
...
bool VideoSourceMulti::SavePoses(const SE3Map& mPoses)
{
  ...
  for(SE3Map::const_iterator se3_it = mPoses.begin(); se3_it != mPoses.end(); ++se3_it)
  {
    ...
    ros::ServiceClient client = mNodeHandle.serviceClient<sensor_msgs::SetCameraInfo>(std::string(cameraName + "/" + mSetInfoTopic));
